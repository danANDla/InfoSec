...
class SignatureChecker(
    ...
    private fun printAllPoints(point: EllipticCurvePoint){
        var i = 0
        i++
        print(String.format("%2d. (%d,%d)", i, point.x, point.y))

        var p = pointAddition(point, basePoint, primeNumber, a)
        while(p != point){
            i++
            print(String.format("%2d. (%d,%d)", i, p.x, p.y))
            p = pointAddition(p, basePoint, primeNumber, a)
        }
    }
